...
if __name__ == "__main__":
    ...
    spct1 = K * (
        (alpha / (freq * freq + f_slow * f_slow))
        + (1.0 - alpha) / (freq * freq + f_fast * f_fast)
    )
    ...
    fpath = (
        r"C:\Research\Spontaneous_activity\Rate_model\CriticalSlowDown\data\figdata.mat"
    )
    d = loadmat(fpath)
    ...
    lettersize = 24
    plt.semilogx(
        freq, spct_db1, label="Lorenzian - Sub Critical", linewidth=3.5, color="green"
    )  # plt.plot(freqs, spct_db)
    plt.semilogx(
        freq, spct_db2, label="Lorenzian - Near Critical", linewidth=3.5, color="orange"
    )
    plt.semilogx(
        X[0], Y[0], label="Norman - Recall", linewidth=3, alpha=0.5, color="red"
    )  # plt.plot(X[0], Y[0])
    plt.semilogx(
        X[1], Y[1], label="Norman - Resting state", linewidth=3, alpha=0.5, color="blue"
    )
