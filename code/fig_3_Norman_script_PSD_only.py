...
from utils import butter_lowpass_filter, butter_highpass_filter, butter_bandpass_filter
...
lettersize = 28
plt.semilogx(freqs1, mean_spct_block1, label='Simulation - rest state', linewidth=4, color='green')  # plt.plot(freqs, spct_db)
plt.fill_between(freqs1, mean_spct_block1 - var_spct_block1, mean_spct_block1 + var_spct_block1, color='green', alpha=0.4)
plt.semilogx(freqs2, mean_spct_block2, label='Simulation - recall', linewidth=4, color='orange')
plt.fill_between(freqs2, mean_spct_block2 - var_spct_block2, mean_spct_block2 + var_spct_block2, color='orange', alpha=0.4)
plt.semilogx(X[0], Y[0], label='Norman - Recall', linewidth=2, alpha=0.9, color='red')  # plt.plot(X[0], Y[0])
plt.semilogx(X[1], Y[1], label='Norman - Resting state', linewidth=2, alpha=0.9, color='blue')
...
low_fr_cutoff = 0.5
range_spect1_range_low = np.sum(np.asarray(spct_blocks1)[:, np.logical_and(freqs1 > 0, freqs1 <= low_fr_cutoff)], axis=1)
range_spect2_range_low = np.sum(np.asarray(spct_blocks2)[:, np.logical_and(freqs1 > 0, freqs1 <= low_fr_cutoff)], axis=1)
diff_spect_range_low = (range_spect2_range_low - range_spect1_range_low) / sum(freqs1 < low_fr_cutoff)
...
one_tailed_p_value=float("{:.6f}".format(p_value/2)) # Since alternative hypothesis is one tailed - need to divide p value by 2.
...
filenames_additive_noise = ['results/fig3/blocks_by_additive_noise_run_1_seed_45_additive_10.pkl',
...
fig = plt.figure(1, figsize=(10, 10))
lettersize = 24
plt.semilogx(freqs1, spct_blocks1[0], label='Simulation - rest state', linewidth=2, color='green')  # plt.plot(freqs, spct_db)
plt.semilogx(freqs2, spct_blocks2[0], label='Simulation - recall - additive noise 10', linewidth=2, color=[1.0, 0.9, 0.25])
plt.semilogx(freqs2, spct_blocks2[1], label='Simulation - recall - additive noise 15', linewidth=2, color=[1.0, 0.85, 0.25])
plt.semilogx(freqs2, spct_blocks2[2], label='Simulation - recall - additive noise 25', linewidth=2, color=[1.0, 0.8, 0.25])
plt.semilogx(freqs2, spct_blocks2[3], label='Simulation - recall - additive noise 50', linewidth=2, color=[1.0, 0.75, 0.25])
plt.semilogx(X[0], Y[0], label='Norman - Recall', linewidth=6, alpha=0.9, color='red')  # plt.plot(X[0], Y[0])
plt.semilogx(X[1], Y[1], label='Norman - Resting state', linewidth=6, alpha=0.9, color='blue')
plt.xlabel('Freq. (Hz)', fontsize=lettersize*1.5)
plt.gcf().subplots_adjust(bottom=0.15)
plt.ylabel('Power (dB)', fontsize=lettersize*1.5)
