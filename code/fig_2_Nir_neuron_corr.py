...
node_activities = (node_activities - node_activities.mean(axis=1, keepdims=True)) / node_activities.std(axis=1, keepdims=True)
node_activities_filt = butter_bandpass_filter(node_activities, 0.00001, 0.1, fs)
node_activities_filt = (node_activities_filt - node_activities_filt.mean(axis=1, keepdims=True)) / \
...
for i in range(1,40,2):
    ac = correlate(node_activities[i, :], node_activities[i, :], mode='same', method='auto')/size
    auto_corr_arr.append(ac)
    cc = correlate(node_activities[i, :], node_activities[i+1, :], mode='same', method='auto')/size
...
plt.plot(x_gamma, df_corr.values[:,1], label='Max correlation', linewidth=4, color='blue')  # max neuroal correlation
plt.plot(x_gamma, df_corr.values[:,2], label='Mean correlation', linewidth=4, color='green')  # mean neuronal correlation
...
ax1 = plt.subplot(211)
ax1.plot(x_gamma, df_corr.values[:,1], label='Max correlation', linewidth=4, color='blue')  # max neuroal correlation
ax1.plot(x_gamma, df_corr.values[:,2], label='Mean correlation', linewidth=4, color='green')  # mean neuronal correlation
...
ax2 = plt.subplot(212)
ax2.plot(x_gamma, np.divide(np.log(df_corr.values[:,3]),np.log(df_corr.values[:,4])), label='Auto-to-cross corr ratio', linewidth=4, color='orange')
...
df_corr = pd.read_csv('C:/Research/Spontaneous_activity/Rate_model/CriticalSlowDown/research-critical-slow-down/data/fig_SI_network_correlation_by_N.csv')
df_corr_2 = pd.read_csv('C:/Research/Spontaneous_activity/Rate_model/CriticalSlowDown/research-critical-slow-down/data/fig_SI_network_correlation_by_N_2.csv')
...
fig = plt.figure(figsize=(8, 8))
# ax2 = ax.twinx()
plt.plot(N, df_corr.values[:,2], label='Max correlation', linewidth=4, color='blue')  # max neuroal correlation
...
ax2 = ax1.twinx()
ax1.plot(N, df_corr.values[:,2], label='Max correlation', linewidth=4, color='blue')  # max neuroal correlation
ax1.plot(N, df_corr.values[:,3], label='Mean correlation', linewidth=4, color='green')
plt.yticks(fontsize=lettersize)

ax2.plot(N, df_corr.values[:,6], label='Normalized sum power', linewidth=4, color='orange')
plt.yticks(fontsize=lettersize)
plt.xticks(fontsize=lettersize)
